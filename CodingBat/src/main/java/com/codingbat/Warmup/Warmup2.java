...
public class Warmup2 {
    ...
    public String stringSplosion(String str) {
        String s = "";
        for (int i = 0; i < str.length(); i++) s += str.substring(0, i);
        return s + str;
    }
    ...
    public boolean arrayFront9(int[] nums) {
        ...
        for (int i = 0; i < end; i++) if (nums[i] == 9) return true;
        return false;
    }
    ...
    public int array667(int[] nums) {
        int count = 0;
        for (int x = 0; x < nums.length - 1; x++) if (nums[x] == 6 && (nums[x + 1] == 6 || nums[x + 1] == 7)) count++;
        return count;
    }
